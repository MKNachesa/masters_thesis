...
def split_audio_by_speech(df, vp_dir,#speaker_model=None,
    ...
    filename_dokid = df["filename"].iloc[0]     # e.g. GR01SKU25/2442210140028308521_aud.wav
    filename = (
        Path(filename_dokid).parent / Path(filename_dokid).stem
    )  # Filename without extension.
    input_file = os.path.join(audio_dir, filename_dokid)    # e.g. /file/to/audio/dir/GR01SKU25/2442210140028308521_aud.wav

    dokid = df["dokid"].iloc[0]

    if len(os.listdir(os.path.join(audio_dir, dokid))) > 1:
        return df

    try:
        sound = AudioSegment.from_mp3(os.path.join(audio_dir, filename_dokid))
    except:
        # exceptions occur when the file is too big too open
        # here the file is split into two, then rejoined together to still be able to be processed
        last_ts = df.timestamps_full.iloc[-1][1]
        middle = last_ts/2
        try:
            last_before_middle = list(filter(lambda x: x[1] < middle, df.timestamps_full.tolist()))[-1][1]
            cutoff = (last_before_middle+middle)/2
        except:
            first_after_middle = list(filter(lambda x: x[0] > middle, df.timestamps_full.tolist()))[0][0]
            cutoff = (first_after_middle+middle)/2
        cutoff = to_timestamp(cutoff)
        outfile1 = f"{input_file.strip('.mp3')}_1.mp3"
        outfile2 = f"{input_file.strip('.mp3')}_2.mp3"

        subprocess.call(["ffmpeg", "-i", input_file, "-to", str(cutoff), "-c", "copy", outfile1])
        subprocess.call(["ffmpeg", "-i", input_file, "-ss", str(cutoff), "-c", "copy", outfile2])

        try:
            sound1 = AudioSegment.from_mp3(outfile1)
            sound1 = sound1.set_frame_rate(16000)
            sound1 = sound1.set_channels(1)

            sound2 = AudioSegment.from_mp3(outfile2)
